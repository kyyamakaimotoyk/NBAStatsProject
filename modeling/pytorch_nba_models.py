...
def main():
    ...
    # =========================================================================
    # COMPARISON WITH SKLEARN BASELINES (pulled from model_registry, not hardcoded)
    # =========================================================================
    print("\n" + "="*70)
    print("COMPARISON: PyTorch vs Scikit-learn (RF baseline from model_registry)")
    print("="*70)
    ...
    try:
        ...
        from core.model_registry import get_current_model
        engine = get_engine()
        rf_clf = get_current_model(engine, 'rf_classifier') or {}
        rf_reg = get_current_model(engine, 'rf_regressor') or {}
        rf_clf_auc = rf_clf.get('test_auc')
        rf_clf_acc = rf_clf.get('test_accuracy')
        rf_reg_mae = rf_reg.get('test_mae')
        rf_reg_r2 = rf_reg.get('test_r2')
        engine.dispose()
        print(f"""
    Scikit-learn Random Forest baseline (current registered version):
    -----------------------------------------------------------------
    Classification: Accuracy={rf_clf_acc}, AUC={rf_clf_auc}
    Regression:     MAE={rf_reg_mae}, R2={rf_reg_r2}
        """)
    except Exception as e:
        print(f"  (Could not fetch RF baseline from model_registry: {e})")

    print(f"""
    PyTorch Neural Network Results (this run):
    ------------------------------------------
    Classification: Accuracy={test_accuracy:.3f}, AUC={test_auc:.3f}
    Regression:     MAE={test_mae:.2f} points, R2={test_r2:.3f}
    """)

    print("Verdict:")
    if rf_clf_auc is not None:
        if test_auc > rf_clf_auc:
            print(f"  Classification: Neural Network WINS (AUC {test_auc:.3f} vs RF {rf_clf_auc:.3f})")
        else:
            print(f"  Classification: Random Forest wins (RF AUC {rf_clf_auc:.3f} vs NN {test_auc:.3f})")
    if rf_reg_mae is not None:
        if test_mae < rf_reg_mae:
            print(f"  Regression: Neural Network WINS (MAE {test_mae:.2f} vs RF {rf_reg_mae:.2f})")
        else:
            print(f"  Regression: Random Forest wins (RF MAE {rf_reg_mae:.2f} vs NN {test_mae:.2f})")
